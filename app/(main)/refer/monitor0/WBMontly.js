...
import { wirebreak_daily, wirebreak_monthly,withAuth } from "../../../api/kcvl"

const WBMonthly = ({ YYMM }) => {
    ...
    const _datelist = generateYearMonthList(_dateYY)
    ...
    const fetchData = async (isYYMM) => {
        try {
            ...
            const _resultYYMM = await wirebreak_monthly(_paramsYYMM)
            const _resultYYMMDD = await wirebreak_daily(_paramsYYMMDD);
            // console.log('handleFetchData > _resultYYMM', _resultYYMM.data)
            // console.log('handleFetchData > _resultYYMMDD', _resultYYMMDD.data)

            setResultDT(_resultYYMM.data);

            if (_resultYYMM.data.length > 0) {
                if(_resultYYMMDD.data.length > 0) {
                    let _temp = filterData(_resultYYMM.data, _resultYYMMDD.data)
                    setResultDTfilter(_temp)
                } else {
                    setResultDTfilter(_resultYYMM.data)
                }                            
            } else {
                setResultDTfilter(_resultYYMM.data)
            }
        ...
        <div className="grid mt-0">
            ...
            <div className="col-12">
                ...
                >
                    ...
                    {_datelist.map((day, index) => (
                        <Column key={index} header={day} field={`wb${day}`} body={(rowData) => bodyWB(rowData[`nwb${day}`], rowData[`pq${day}`])} style={{ width: '6%' }}></Column>
                    ))}
                </DataTable>
            </div>
            ...
            <div className="col-12">
                ...
                >
                    ...
                    <Column header="Construction" field="Construction" sortable></Column>
                    {_datelist.map((day, index) => (
                        <Column key={index} header={day} field={`pq${day}`} body={(rowData) => bodyPQ(rowData[`pq${day}`])} style={{ width: '6%' }}></Column>
                    ))}
            ...
            <div className="col-12">
                ...
                >
                    ...
                    <Column header="Construction" field="Construction" sortable></Column>
                    {_datelist.map((day, index) => (
                        <Column key={index} header={day} field={`nwb${day}`} body={(rowData) => bodyNWB(rowData[`nwb${day}`])} style={{ width: '6%' }}></Column>
                    ))}
                </DataTable>
            </div>
        </div>
...
export default withAuth(WBMonthly);